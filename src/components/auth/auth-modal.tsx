      ...
      <DialogContent className="rounded-3xl sm:max-w-2xl overflow-hidden p-6 shadow-modal max-h-[calc(100dvh-3rem)] bg-[linear-gradient(207.31deg,rgb(238,231,250)_3.83%,rgb(245,245,245)_56.46%,rgb(252,218,202)_112.15%)] dark:bg-[linear-gradient(207.31deg,var(--surface)_3.83%,var(--surface-muted)_56.46%,var(--surface-elevated)_112.15%)]">
        ...
        {["forgot-password", "otp", "reset-password"].includes(authModalStep) && !resetSuccess && (
          <button
            type="button"
            onClick={handleBack}
            className="absolute start-6 top-6 flex h-8 w-8 items-center justify-center rounded-full bg-surface-muted hover:bg-accent text-foreground transition-all cursor-pointer z-20 shadow-xs"
          >
        ...
        <div className="flex flex-col items-center justify-center pt-2 pb-5 z-10 relative">
        ...
        <div className="relative z-10 flex min-h-0 w-full flex-col px-8 pb-8 pt-2 sm:px-10 sm:pb-10 no-scrollbar scrollbar-hide max-h-[calc(100dvh-10rem)] overflow-y-auto">
          {/* STEP: LOGIN */}
          {authModalStep === "login" && (
